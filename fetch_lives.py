...
# 1. 頻道分類清單 (建議檢查這些網址，有些頻道已更改 Handle)
CATEGORIES = {
    "跨年,#genre#": {
...
LANDMARK_MAP = {
    ...
    "Baling": "巴陵大橋", "Shihmen Reservoir": "石門水庫", "Sanxiantai": "三仙台"
}
...
def extract_best_title(v_title, nickname):
    ...
    chinese_found = "".join(re.findall(r'[\u4e00-\u9fa5]+', clean_title))
    
    # 移除噪音
    for n in ["即時影像", "直播", "官方", brand]:
        chinese_found = chinese_found.replace(n, "")
        
    landmark = chinese_found if len(chinese_found) >= 2 else ""
    if not landmark:
        for eng, chi in LANDMARK_MAP.items():
            if eng.lower() in v_title.lower(): landmark = chi; break
            
    return f"【{brand}】{landmark if landmark else clean_title[:10]}"
...
def get_live_info():
    ...
    ydl_opts = {
        ...
        'skip_download': True,
        'playlist_items': '1-10',
        'ignoreerrors': True, # 忽略單個頻道錯誤，繼續執行下一個
        'no_warnings': True,
        'extra_headers': {
    ...
    for genre, channels in CATEGORIES.items():
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            for nickname, base_url in channels.items():
                # 策略：嘗試 streams 頁面，若出錯 yt-dlp 會自動回退至主頁面
                target_urls = [f"{base_url}/streams", base_url]
                        ...
                        for entry in entries:
                            if entry and (entry.get('live_status') == 'is_live' or entry.get('is_live')):
                                ...
                                if v_url not in all_seen_urls:
                                    ...
                                    found_live = True
                        if found_live: break # 如果在 streams 找到直播，就不必再找主頁
                    except Exception as e:
                        continue # 嘗試下一個 URL 路徑
